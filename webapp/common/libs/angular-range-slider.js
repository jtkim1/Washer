  ...
  module.directive('rangeSlider', [function(){
    ...
    return {
      ...
      link : function(scope, element, attrs) {
          //console.log("=-==-=-=-=-=-=-=-=-creating the range slider directive = " + scope.ngModel);
        ...
        var updateCalculations = function() {
          var isDegF = scope.isDegF;
          scope.degChar = (isDegF === true || isDegF === 'true')? 'F' : 'C';
          ...
          barWidth = bar[0].offsetWidth + (1 * getBaseFont());     //k.amit1 added (1 * getBaseFont()) to compensate 1rem selection space
          minOffset = 0;
        ...
        var updateDOM = function () {
          ...
          var setPointers = function () {
            ...
            offset(lowBubble, pixelize(parseInt(pixelsToOffset(newLowValue)) - (lowBubble[0].offsetWidth / 2) + handleHalfWidth));
          ...
          var bind = function (handle, bubble, ref, events) {
            ...
            bubble.bind(events.start, onStart);
          };
        ...
        scope.$on("$destroy", function() {
            //console.log("+++++++++++++++++++destroying range slider directive++++++++++++++++++++");
        });
      ...
      template :  '<div class="bar"><div class="selection"></div></div>' +
                  ...
                  '<div class="bubble value low" ng-if="false">{{ ngModelLow }}<sup class="min-font">&deg;{{degChar}}</sup></div>' +
