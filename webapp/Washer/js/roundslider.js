...
(function ($, window, undefined) {
    ...
    RoundSlider.prototype = {
        ...
        _createLayers: function () {
            ...
            this.lastBlock = createElement("span.rs-block").css({"padding": '0.55rem'});
        ...
        _setRadius: function () {
            ...
            var style = {"height": '11.1rem', "width": '11.1rem'};
        ...
        _refreshHandle: function () {
            ...
            this._handles().css({height: '1.2rem', width: '1.2rem'});
        },
        ...
        _elementDown: function (e) {
            ...
            if ($target.hasClass("rs-handle")) {
                ...
                outerDistance = outerDistance + 5;
                innerDistance = innerDistance - 5;
        ...
        _handleKeyDown: function (e) {
            ...
            e.preventDefault();
            if (key === 38 || key === 37)
                val = this._round(this._limitValue(h.value + this.options.step));  // Up || Left Key
            else if (key === 39 || key === 40)
                val = this._round(this._limitValue(h.value - this._getMinusStep(h.value))); // Right || Down Key
            else if (key === 36)
                val = this._getKeyValue("Home"); // Home Key
            else if (key === 35)
                val = this._getKeyValue("End"); // End Key
        ...
        _updateARIA: function (value) {
            ...
            updateKitchenTimerValue(value, this.options.tab);
        ...
        _updateHidden: function () {
            ...
            if (this._hiddenField !== undefined) {
        ...
        _analyzeModelValue: function () {
            ...
            if (this._rangeSlider) {
                if (typeof val === "string") {
                    if (parts.length >= 2)
                        t = (isNumber(parts[0]) ? parts[0] : min) + "," +
